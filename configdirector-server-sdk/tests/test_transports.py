...
from configdirector._version import SERVER_SDK_IDENTITY, SdkIdentity
...
def options(url: str, sink: Sink, logger: RecordingLogger, **overrides: Any) -> TransportOptions:
    return TransportOptions(
        ...
        sdk_identity=overrides.pop("sdk_identity", SERVER_SDK_IDENTITY),
        logger=logger,
        on_bundle=sink,
        **overrides,
    )
...
class TestUserAgent:
    ...
    def test_polling_identifies_a_wrapper_by_its_own_name_and_version(
        self, serve: Callable[[Handler], _Server], sink: Sink, logger: RecordingLogger
    ) -> None:
        server = serve(lambda request: respond(request, 200, bundle_json("a")))
        identity = SdkIdentity(sdk_name="wrapper-tests", sdk_version="9.8.7")
        transport = PollingTransport(options(server.url, sink, logger, sdk_identity=identity))
        try:
            transport.connect(5.0)
        finally:
            transport.close()

        assert server.headers[0].get("User-Agent") == "wrapper-tests/9.8.7"

    def test_streaming_identifies_a_wrapper_by_its_own_name_and_version(
        self, serve: Callable[[Handler], _Server], sink: Sink, logger: RecordingLogger
    ) -> None:
        def handle(request: http.server.BaseHTTPRequestHandler) -> None:
            sse_headers(request)
        ...
        server = serve(handle)
        identity = SdkIdentity(sdk_name="wrapper-tests", sdk_version="9.8.7")
        transport = StreamingTransport(options(server.url, sink, logger, sdk_identity=identity))
        transport.connect(5.0)
