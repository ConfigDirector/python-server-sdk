...
`configdirector-server-sdk-v<version>`; releases up to 1.1.0 were tagged `v<version>`.
...
- The telemetry report now includes the application name and version supplied in `Metadata`, the
  same way config requests already did, so evaluations can be shown per app in the dashboard's
  activity graphs.
...
- Packages built on this SDK, starting with `configdirector-openfeature-server-provider`, now identify themselves by their own name and version in the `User-Agent` header and in telemetry. Only packages the SDK knows about can do so; the public API is unchanged.
