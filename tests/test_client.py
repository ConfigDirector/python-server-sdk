...
@pytest.fixture
def ready_client(client: _ConfigDirectorClient) -> _ConfigDirectorClient:
    client.initialize()
    return client


class TestInitialize:
    def test_marks_the_client_ready_once_config_state_arrives(self, client: _ConfigDirectorClient) -> None:
    ...
    def test_rejects_a_non_positive_timeout(self, client: _ConfigDirectorClient, timeout: float) -> None:
        with pytest.raises(ConfigDirectorValidationError, match="timeout"):
            client.initialize(timeout=timeout)

    def test_rejects_use_after_close(self, client: _ConfigDirectorClient) -> None:
        client.close()

        with pytest.raises(ConfigDirectorValidationError, match="closed"):
            client.initialize()

    def test_emits_configs_updated_before_client_ready(self, client: _ConfigDirectorClient) -> None:
        events: list[str] = []
...
class TestGetValue:
    ...
    def test_returns_the_default_when_the_key_is_unknown(
        self, ready_client: _ConfigDirectorClient, default: Any
    ) -> None:
        assert ready_client.get_value("any-key", default) == default

    def test_returns_the_default_before_initialization(self, client: _ConfigDirectorClient) -> None:
    ...
    def test_rejects_a_blank_config_key(self, ready_client: _ConfigDirectorClient, config_key: str) -> None:
        with pytest.raises(ConfigDirectorValidationError, match="config key"):
            ready_client.get_value(config_key, "fallback")

    def test_rejects_a_none_default(self, ready_client: _ConfigDirectorClient) -> None:
    ...
    def test_type_errors_are_also_builtin_type_errors(self, ready_client: _ConfigDirectorClient) -> None:
        with pytest.raises(TypeError, match="Invalid default value of type"):
            ready_client.get_value("any-key", {1, 2})  # type: ignore[type-var]

    def test_emits_a_config_evaluated_event(self, ready_client: _ConfigDirectorClient) -> None:
        events: list[ConfigEvaluatedEvent] = []
    ...
    def test_reports_client_not_ready_before_initialization(self, client: _ConfigDirectorClient) -> None:
        events: list[ConfigEvaluatedEvent] = []
        client.on("config_evaluated", events.append)
...
class TestGetAllConfigs:
    def test_returns_nothing_before_initialization(self, client: _ConfigDirectorClient) -> None:
    ...
    def test_does_not_emit_evaluation_events(self, ready_client: _ConfigDirectorClient) -> None:
        events: list[ConfigEvaluatedEvent] = []
        ready_client.on("config_evaluated", events.append)
...
class TestWatch:
    def test_returns_a_subscription(self, ready_client: _ConfigDirectorClient) -> None:
    ...
    def test_closing_the_subscription_stops_the_watch(self, ready_client: _ConfigDirectorClient) -> None:
        subscription = ready_client.watch("any-key", False, lambda _value: None)
    ...
    def test_closing_is_idempotent(self, ready_client: _ConfigDirectorClient) -> None:
        subscription = ready_client.watch("any-key", False, lambda _value: None)
    ...
    def test_the_subscription_is_a_context_manager(self, ready_client: _ConfigDirectorClient) -> None:
    ...
    def test_closing_one_subscription_leaves_an_identical_one(
        self, ready_client: _ConfigDirectorClient
    ) -> None:
    ...
    def test_subscriptions_compose_with_exit_stack(self, ready_client: _ConfigDirectorClient) -> None:
    ...
    def test_rejects_a_non_callable_callback(self, ready_client: _ConfigDirectorClient) -> None:
    ...
    def test_rejects_an_invalid_default(self, ready_client: _ConfigDirectorClient) -> None:
        with pytest.raises(ConfigDirectorTypeError, match="must not be None"):
            ready_client.watch("any-key", None, lambda _value: None)  # type: ignore[type-var]

    def test_unwatch_removes_a_single_callback(self, ready_client: _ConfigDirectorClient) -> None:
    ...
    def test_unwatch_is_a_no_op_for_an_unknown_key(self, ready_client: _ConfigDirectorClient) -> None:
        ready_client.unwatch("never-watched")

    def test_unwatch_all_removes_every_watcher(self, ready_client: _ConfigDirectorClient) -> None:
...
class TestEvents:
    def test_on_returns_a_closable_subscription(self, client: _ConfigDirectorClient) -> None:
        calls: list[str] = []
    ...
    def test_the_event_subscription_is_a_context_manager(self, client: _ConfigDirectorClient) -> None:
        calls: list[str] = []
    ...
    def test_off_removes_a_single_handler(self, client: _ConfigDirectorClient) -> None:
        calls: list[str] = []
    ...
    def test_off_without_a_handler_removes_every_handler(self, client: _ConfigDirectorClient) -> None:
        calls: list[str] = []
    ...
    def test_rejects_an_unknown_event_name(self, client: _ConfigDirectorClient, method: str) -> None:
    ...
    def test_rejects_a_non_callable_handler(self, client: _ConfigDirectorClient) -> None:
        with pytest.raises(ConfigDirectorTypeError, match="callable"):
            client.on("client_ready", "not-callable")  # type: ignore[call-overload]

    def test_a_raising_handler_does_not_break_the_caller(self, client: _ConfigDirectorClient) -> None:
...
class TestClose:
    def test_clears_readiness_and_subscribers(self, ready_client: _ConfigDirectorClient) -> None:
        calls: list[str] = []
        ready_client.watch("any-key", False, lambda _value: None)
    ...
    def test_closes_the_transport(
        self, ready_client: _ConfigDirectorClient, transports: TransportRecorder
    ) -> None:
        ready_client.close()
    ...
    def test_is_idempotent(self, ready_client: _ConfigDirectorClient) -> None:
        ready_client.close()
        ready_client.close()
...
class TestTelemetry:
    def test_records_every_evaluation(
        self, client: _ConfigDirectorClient, transports: TransportRecorder, telemetry: TelemetryRecorder
    ) -> None:
...
def _watchers(client: _ConfigDirectorClient, config_key: str) -> list[Any]:
...
class TestValueIds:
    ...
    def _evaluations(self, client: _ConfigDirectorClient) -> list[ConfigEvaluatedEvent]:
        events: list[ConfigEvaluatedEvent] = []
        client.on("config_evaluated", events.append)
        return events
    ...
    def test_a_missing_config_gets_a_computed_id(self, ready_client: _ConfigDirectorClient) -> None:
